...
import { MessageSquare, X, Send, Bot, ShieldQuestion, ChevronRight } from "lucide-react";
...
export function ChatbotUI() {
  ...
  return (
    <div className="fixed bottom-8 left-8 z-[100]">
      {!isOpen && (
        <button
          onClick={() => { setIsOpen(true); initialBotMessage(); }}
        ...
        <div className="w-[350px] max-w-[90vw] h-[550px] bg-white dark:bg-zinc-900 border-4 border-black dark:border-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] flex flex-col overflow-hidden animate-reveal">
          ...
          <div className="p-4 border-b-4 border-black dark:border-white bg-black dark:bg-white text-white dark:text-black flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="font-black uppercase text-xs tracking-widest italic">Privacy AI</span>
            </div>
          ...
          {/* Messages */}
          <div ref={scrollRef} className="flex-grow p-4 overflow-y-auto space-y-4 bg-zinc-50 dark:bg-zinc-950">
            {messages.map((msg, i) => (
              <div key={i} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                <div className={`max-w-[85%] p-3 border-2 border-black dark:border-white font-bold text-sm ${
                  msg.role === "user" 
          ...
          <div className="p-4 border-t-4 border-black dark:border-white flex gap-2 bg-white dark:bg-zinc-900">
            <input
              ...
              placeholder={language === "vi" ? "Hỏi về quyền riêng tư..." : "Ask about privacy..."}
